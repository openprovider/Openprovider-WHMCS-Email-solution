    ...
    try {
        ...
        if ($domainDetail->domain) {
            ...
            if (isset($_POST['formAction']) && $_POST['formAction'] == 'addMailbox' && isset($_POST['full_name']) && isset($_POST['mailbox'])  && isset($_POST['password'])) {
                ...
                // [INTGRTNS-502] Use live contracted limit from /v1beta/mailcow/statistics

            $statistics = $apiCall->getCurl($token, '/v1beta/mailcow/statistics', 'Get Statistics');

            if ($statistics['httpcode'] == 200) {

                $limit = (int)$statistics['result']->data->total_count;

                $countTotalMailBox = (int)$statistics['result']->data->used_count;

            } else {

                logModuleCall('Email Solution', 'Get Statistics', [], 'Statistics API failed, using local DB fallback', []);

                $limit = (int)Capsule::table('mod_email_solution_product')->where('service_id', $params['serviceid'])->value('mailcow');

                $countTotalMailBox = is_array($getMailBox['result']->data->results) ? count($getMailBox['result']->data->results) : 0;

            }



            if ($limit > $countTotalMailBox && $limit > 0) {
            ...
            if (isset($_POST['formAction']) && $_POST['formAction'] == 'editMailbox' && isset($_POST['editMailId']) && isset($_POST['edit_password'])  && isset($_POST['edit_full_name'])) {
                ...
                // TODO: implement edit mailbox handler (INTGRTNS-502)
